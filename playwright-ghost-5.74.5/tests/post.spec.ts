...
import { getAprioriData, getMockaroJson } from '../utils/getMockaroJson';
...
test.describe('Posts - A priori data', () => {
  ...
  test('Crear un nuevo post con solo título y descripción de emojis', async ({ page }) => {
    const postsData = await getAprioriData('posts.json');
    const aprioriData = await getMockaroJson('/settings/emoji-apropri.json');
    const contenido = aprioriData[0].titleEmoji;
  ...
  test('Crear un nuevo post con solo título y descripción de SQL inyección', async ({ page }) => {
    const postsData = await getAprioriData('posts.json');
    const aprioriData = await getMockaroJson('/settings/sql-inyeccion.json');
    const contenido = aprioriData[0].sqlInyeccion;
  ...
  test('Crear un nuevo post con título de sql inyección', async ({ page }) => {
    const aprioriData = await getMockaroJson('/settings/sql-inyeccion.json');
    const titulo_post = aprioriData[0].sqlInyeccion;
    let paso = 1;
  ...
  test('No es posible Crear un nuevo post con título de emoji', async ({ page }) => {
    const aprioriData = await getMockaroJson('/settings/emoji-apropri.json');
    const titulo_post = aprioriData[0].titleEmoji;
    ...
    await test.step('When: El usuario hace clic en "New post', async () => {
      ...
      await page.getByRole('link', { name: 'New post' }).click();
      await screenshotPagePath(page, 'post', 'Crear_un_nuevo_post_con_título_de_emoji', paso++);
    });
    ...
    await test.step('Then: Se verifica que el botón de Post no aparece y no publica', async () => {
      // abre la new page
  ...
  test('Crear un post con excerpt que contiene sql inyección', async ({ page }) => {
    const postsData = await getAprioriData('posts.json');
    const titulo_post = postsData.title;
    const contenido = postsData.content;

    const SQlData = await getMockaroJson('/settings/sql-inyeccion.json');
    const excerpt = SQlData[0].sqlInyeccion;
  ...
  test('Crear un post con excerpt que contiene emojis', async ({ page }) => {
    const postsData = await getAprioriData('posts.json');
    const titulo_post = postsData.title;
    const contenido = postsData.content;

    const emojiData = await getMockaroJson('/settings/emoji-apropri.json');
    const excerpt = emojiData[0].titleEmoji;
...
test.describe('Posts - Random data', () => {
  ...
  test('Crear un nuevo post con url random en el título', async ({ page }) => {
    const titulo_post = `${faker.internet.url()}`;
    let paso = 1;
    ...
    await test.step('When: El usuario hace clic en "New post', async () => {
      ...
      await page.getByRole('link', { name: 'New post' }).click();
      await screenshotPagePath(page, 'post', 'Crear_un_nuevo_post_con_url_en_el_título', paso++);
    });
    ...
    await test.step('And: hace clic en confirmación de publicación', async () => {
      ...
      // Hace clic en el botón
      await page.click(buttonSelector);
      await screenshotPagePath(page, 'post', 'Crear_un_nuevo_post_con_url_en_el_título_y_descripción', paso++);
    });
    ...
    await test.step('And: El usuario se dirige al post creado', async () => {
      ...
      // Toma un screenshot si es necesario
      await screenshotPagePath(newPage, 'post', 'Crear_un_nuevo_post_con_url_en_el_título', paso++);
